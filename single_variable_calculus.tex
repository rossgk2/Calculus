...
The reason is that logarithms were first discovered in 1614, far before computers automated away the doing of arithmetic. Mathematicians needed a computational aid to help them multiply large numbers. Since $x \mapsto \log_b(x)$ is a relatively slowly increasing function of $x$, and since $\log_b(xy) = \log_b(x) + \log_b(y)$, logarithms allowed the conversion of multiplication of large numbers into addition of much smaller numbers.

Since logarithms were the perfect tool for the time, mathematicians were in the habit of thinking about the reorganized number line that results from applying the logarithm to every number in the ``original'' number line. If we apply $\log_b$ for some $b > 0$ to the entire real number line, then numbers which are multiplicative factor apart in the ``original'' number line are now an additive difference apart in the reorganized number line. That is, constant ratios between numbers in the ``original'' number line correspond to constant differences between numbers in the reorganized line:

\begin{align*}
    \frac{x}{y} = c \iff \log_b(x) - \log_b(y) = c.
\end{align*}

Mathematicians called numbers in the new number line \textit{logarithms}, since this means ``ratio-number'' in Greek.
