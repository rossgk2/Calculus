...
Now we derive the inverse product rule. It's most commonly referred to as ``integration by parts'' in reference to the fact that it converts the problem of integrating a product into the problem of computing two integrals, where the second depends on the first. One has to do the first ``part'', and then the second.
...
\begin{align*}
	fg = \int (f'g + g'f) = \int f'g + \int g'f
\end{align*}
...
And then applying the inverse chain rule yields the integration by parts formula
...
For some reason it is traditional to state the formula with the letters $u$ and $v$ instead of $f$ and $g$, like this:

\begin{align*}
	\text{``}\int u \spc dv = uv - \int v \spc du\text{''}.
\end{align*}

While the above two formulas \textit{may} be memorable, they slightly obscure how to actually apply the rule, mostly because it is not obvious, when one has functions $h$ and $k$, that they are supposed to interpret integrals like $\int h(x) k(x) dx$ to be of the form $\int f(x) dg(x)$.

For the most straightforward presentation of the idea, again consider the equation we derived above,
...
This presentation is very clearly the integral of a product of functions, and its explicitness makes it easy to explain how to apply the rule. The explanation is as follows. Since $f'$ and $\int g$ appear on the right side, then, when we are trying to compute $\int hk$ for functions $h$ and $k$, we should choose $f$ and $g$ with $hk = fg$, where $f$ is whichever of $h, k$ is easier to differentiate, and $g$ is whichever of $h, k$ is easier to integrate.