...
\textbf{The first half of calculus is about using infinitesimals to compute \textit{change} from \textit{quantity}.} If we want to measure the rate at which the output of a function $f$ is changing in response to a changing input, \textit{instantaneously} at a particular input $x$, we compare the ratio of an infinitesimal change in output, $df(x)$, to the infinitesimal change in input that caused it, $dx$, and compute the ratio $\frac{df(x)}{dx}$. Geometrically, this ratio is the slope of the tangent line passing through $(x, f(x))$.

(The ratio $\frac{df(x)}{dx}$ is called the \textit{derivative} of $f$ at $x$. Sometimes, it is denoted as $f'$. Computing the derivative of $f$ is called \textit{differentiating} $f$. Thus, the first half of calculus is known as \textit{differential calculus}.)

\textbf{The second half of calculus is about using infinitesimals to compute \textit{quantity} from \textit{change}.} If we want to determine how much total change has been caused by a rate of change $f'$ that itself changes over an input window $[a, b]$, we apply the \textit{fundamental theorem of calculus}. That is, we think of the rate of change as being the aggregation of many rates of change that \textit{don't} change over time, where each of these rates occurs over an infinitesimally wide window of inputs, and then compute the sum of all the little changes $df(x_1), ..., df(x_i), ...$ that result. Geometrically, this sum is the area between $f$ and the $x$-axis, and between the lines $x = a$ and $x = b$.

(The mentioned sum is denoted by $\int^b_a df(x)$. It is equal to $\int^b_a \frac{df(x)}{dx}\, dx = \int^b_a f'(x)\, dx$, and is called the \textit{integral} of $f'$ over $[a, b]$. Computing the integral of $f'$ is called \textit{integrating} $f'$. Thus, the second half of calculus is known as \textit{integral calculus}.)

\textbf{The true power of infinitesimals is that they provide a means to derive convenient formulas for these calculations, so that it is not necessary to tediously compute ratios or sums.} If there were no such formulas, then the fundamental theorem of calculus would be basically useless! But, if one knows the formulas, they merely ``calculate'', and apply the formulas. Thus the name \textit{calculus}.
...
A completely valid theory of calculus was only first established in the 1800s, \textit{after} the main ideas mentioned above had been in use for more than a hundred years! The solution was to replace each infinitesimal, which was an "infinitely small number", with a sequence of \textit{actual} numbers that converge to a \textit{limit} of zero. This limit formalism for calculus took root... and is what's used today\footnotemark!

\footnotetext{And then, an approach to formally defining infinitesimals, called \textit{nonstandard analysis}, was at last discovered in the late 1900s. Unfortunately for students who are dogmatically told by unimaginative teachers that "limits are the only way", it didn't catch on. This is because the concepts underlying infinitesimals in nonstandard analysis are extremely difficult, and are used almost nowhere other in mainstream math and physics, while the notion of "limit" is comparatively easy, and is used in plenty of places in math and physics.}
...
Since $\log_b$ is the inverse of $x \mapsto b^x$, we have the algebraic relationship
...
Also because $\log_b$ is the inverse of $x \mapsto b^x$, the graph of $\log_b$ is the reflection of the graph of $x \mapsto b^x$ about the line $y = x$:
...
\begin{itemize}
    \item For all $b > 0$, since the domain and range of $x \mapsto b^x$ are $(-\infty, \infty)$ and $(0, \infty)$, respectively, the domain and range of $\log_b$ are $(0, \infty)$ and $(-\infty, \infty)$, respectively. 
...
That is, the derivative of $b^x$ with respect to $x$ is the evaluation of some function at $b$, times $b^x$. In order to obtain a more useful expression for $\frac{d}{dx} b^x$, we must evaluate the expression $f(b)$. 

First, let us consider the possible values that $f(b)$, which is the velocity of an exponential function at a particular point, can take on. Intuition tells us that we can adjust the base of the exponential function to make its velocity at any particular point as large as we want. The only we cannot do is make the velocity at any point zero, or negative; since $b > 0$, at least some exponential growth will be always be occurring at every input, and thus the velocity will be positive at every input. Thus, we can conclude $f(b) \in (0, \infty)$. But not only that. Since exponential functions are continuous, $f(b)$ must\footnote{See the appendix.} attain every value in $(0, \infty)$.

Now, the natural thing to do (and the key step) is to consider the $b$ for which\footnote{Does such a $b$ even exist? Does $\Big( \frac{d}{dx} b^x \Big)\Big|_0 = \lim_{h \rightarrow 0} \frac{b^h - 1}{h}$ even converge for all $b$? The answer to both questions is yes, but showing so is quite technical. See the appendix.} we have $f(b) = 1$. For the sake of having concrete notation, we will \textit{define} $e$ to be the real number such that $f(e) = 1$. Then

\begin{align*}
    \frac{d}{dx} e^x = f(e) e^x = 1 \cdot e^x = e^x.
\end{align*}

Now, we use change of base for exponential functions in order to determine $\frac{d}{dx} b^x$ for arbitrary $b$ by making use of the fact that we know $\frac{d}{dx} e^x$. We have
...
In some sense, $e$ is the ``most natural base'' because it is the one for which $\frac{d}{dx} e^x = 1 \cdot e^x$. For this reason, the logarithm of base $e$ is called the \textit{natural logarithm}, and is denoted by $\ln$ (for ``logarithmus naturali''). Formally, we define $\ln := \log_e$. So the above is restated as
...
We also define integral notation analogous to the Leibniz notation $\frac{dg(f(x))}{df(x)} := g'(f(x))$:

\begin{align*}
    \int g(f(x)) df(x) := \Big(\int g\Big)\Big|_{f(x)}, \quad \int_a^b g(f(x)) df(x) := \Big(\int_a^b g\Big)\Big|_{f(x)}.
\end{align*}
...
\subsection*{The inverse chain rule (``integration by substitution'')}

Recall that the chain rule for derivatives can be stated as
...
Now, we will think of $g'$ as being ``just a regular function'', and ``not a derivative''. To formalize this, substitute $h := g'$. This implies\footnote{We need to justify that $h$ has an antiderivative here. In order for this to be guaranteed, we actually needed to assume at the beginning that $g$ is \textit{continuously differentiable}, i.e., that $g'$ is continuous. This is a pretty technical detail, so we don't mention it outside this footnote.} $g = \int h$, and we have
...
The theorem is traditionally stated with the substitutions $g = f$ and $f = u$:
...
One important application of the inverse chain rule is the solving of so-called \textit{separable differential equations}, which are equations of the form 

\begin{align*}
    \frac{df(x)}{dx} = f(x) g(f(x)) \text{ for all $x$}.
\end{align*}

The above is an example of a \textit{differential equation}, an equation that involves a function and one or more of its derivatives.

To solve the above equation, we will use the inverse chain rule to ``cancel differentials''.
...
Now use the inverse chain rule to ``cancel the $dx$'s on the left side'':
...
\subsection*{The inverse product rule (``integration by parts'')}

Now we derive the integral counterpart to the product rule for derivatives, which is most commonly- and very uninformatively- referred to as \textit{integration by parts}. 
...
\begin{align*}
	fg = \int (f'g + fg') = \int f'g + \int fg'
\end{align*}

Now subtract one of the integrals on the right side to obtain either
...
Rewriting this in Leibniz notation, we have

\begin{align*}
	\int f(x) \frac{dg(x)}{dx} dx &= f(x) g(x) - \int \frac{df(x)}{dx} g(x) dx.
\end{align*}

And then applying the inverse chain rule yields the inverse product rule formula
...
It's traditional to state the formula with the substitutions $f = u$ and $g = v$, like this:
...
While the above two formulas \textit{may} be memorable, they slightly obscure how to actually apply the rule, mostly because doing so involves some hidden extra steps. Suppose we're trying to compute $\int h(x) k(x) dx$. In doing so, the first hidden step is to interpret the integral to be of the form $\int f(x) dg(x)$. The second hidden step is to realize that since $dg(x)$ appears on the left while $g(x)$ is on the right, it is necessary to compute $g(x) = \int dg(x)$.

For a more straightforward formula, again consider the equation we derived above,
...
This presentation is very clearly the integral of a product of functions, and its explicitness makes it easy to explain how to apply the rule. The explanation is as follows. Since $f'$ and $\int g$ appear on the right side, then, when we have functions $h, k$ and are trying to $\int hk$, we should choose $f$ and $g$ with $hk = fg$, where $f$ is whichever of $h, k$ is easier to differentiate, and $g$ is whichever of $h, k$ is easier to integrate.

A bit confusingly, the inverse product rule is most commonly referred to as \textit{integration by parts}, perhaps because, as we can see above, the calculation involves computing two integrals, and when done by hand is thus best done ``one integral at a time'', or ``part by part''. Pretty much every calculation ever can be performed ``part by part'', though, so ``integration by parts'' is not a very descriptive name.