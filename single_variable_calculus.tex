...
The number $b$ is called the \textit{base}, and $n$ is called the \textit{exponent}. Using this definition, we can derive three ``properties of exponents'' for positive integers $n, m$:
...
The second property could use some simplification. If we define $b^0 := 1$ for all $b$, then the $n = m$ case of the piecewise function can be absorbed into an $n \geq m$ case:
...
In this more general formulation, the second property is a special case of the first\footnote{We have $\frac{b^n}{b^m} = b^n b^{-m} = b^{n + (-m)} = b^{n - m}$.} Thus there are really just \textit{two} properties of exponents:
...
In this notation, $\sqrt[n]{b}$ is called the \textit{$n$th root of $b$}. Please remember, though, that roots are just special notation, and no more than that. It cannot be \textit{proved} that $\sqrt[n]{b} = b^{\frac{1}{n}}$, for this statement is a \textit{definition}.
...
\begin{itemize}
    \item Since the domain and range of $x \mapsto b^x$ are $(-\infty, \infty)$ and $(0, \infty)$, respectivey, the domain and range of $\log_b$ are $(0, \infty)$ and $(-\infty, \infty)$, respectively. 
    \item Since $b^0 = 1$, then $\log_b(1) = 0$.
    \item Since $\lim_{x \rightarrow -\infty} = 0$, then $\lim_{x \rightarrow 0} \log_b(x) = -\infty$.
\end{itemize}
...
Once it is shown that the above ``Properties of exponents'' hold for real numbers, it is easy to show that the following corresponding properties of logarithms immediately hold. For all real numbers $b, x, y > 0$, we have:
...
While the first property first property is easy enough to \textit{prove}, it's more difficult to understand. See the next section for a good interpretation.

For the second property- notice that, for nonnegative values of $y$, it is an immediate consequence of the first property. Since $\log_b(\frac{1}{x}) = -\log_b(x) \iff \frac{1}{x} = b^{-\log_b(x)} \iff \frac{1}{x} = (b^{\log_b(x)})^{-1} = x^{-1} \iff \frac{1}{x} = \frac{1}{x}$, it holds for $y = -1$. From here the first property can be applied again to prove the second property for all integer $y$. And from here, it is simple enough to generalize the second property to all rational $y$. Once we know the second property is true for all rational $y$, the continuity of $\log_b$ when $b > 0$ forces it to be true for all real numbers $y$.

The third property is just straightforwardly a rephrasing of the corresponding property for exponential functions.

\subsubsection*{Why $\log_b(xy) = \log_b(x) + \log_b(y)$}

Here's an explanation of $\log_b(xy) = \log_b(x) + \log_b(y)$. Consider that if $f_C(x) = C b^x$, then $f_C^{-1}(x) = \log_b(\frac{x}{C})$. We can interpret $f_C$ as describing the exponential growth of some quantity, where the initial amount of that quantity is $C$ (since $f(0) = Cb^0 = C$). Since $f_C^{-1}$ is the inverse to $f$, we can interpret $f_C^{-1}(x)$ as describing the amount of time needed to produce $x$ amount of the quantity, assuming that the initial amount of the quantity was $C$ and that the quantity grows by a multiplicative factor of $b$ for every $1$ unit of time:
...
The reason is that logarithms were first discovered in 1614, far before computers automated away the doing of arithmetic. Mathematicians needed a computational aid to help them multiply large numbers. Since $x \mapsto \log_b(x)$ is a relatively increasing function, and since $\log_b(xy) = \log_b(x) + \log_b(y)$, logarithms allowed the conversion of multiplication of large numbers into addition of much smaller numbers.

Logarithms were the perfect tool for the time, so mathematicians were in the habit of thinking about the reorganized set of positive numbers that results from applying the logarithm to every ``original'' positive number. If we apply $\log_b$ for some $b > 0$ to all of the positive numbers, then numbers which are multiplicative factor apart in the ``new'' organization are now an additive difference apart in the ``original'' organization. That is, constant ratios between numbers in the ``original'' organization correspond to constant differences between numbers in the ``original'' organization:

\begin{align*}
    \frac{x}{y} = c \iff \log_b(x) - \log_b(y) = c.
\end{align*}

Mathematicians used the term \textit{logarithm}, which means ``ratio-number'' in Greek, to refer to a number in this new organization of positive numbers, since ``ratio-number'' alludes to this property.

\subsubsection*{How to use logarithm tables}

Specifically, mathematicians had tables of precomputed logarithm values, like this:
...
To multiply two large numbers, like 1234 and 5678, one would first use logarithms to convert multiplication into addition:

\begin{align*}
    1234 \cdot 5678 &= 10^{\log_{10}(1234 \cdot 5678)} = 10^{\log_{10}(1234) + \log_{10}(5678)}.
\end{align*}

Then, one would use the table to approximate the logarithms in the sum. From the table, we can see $\log_{10}(1234) \approx 3.0913$ and $\log_{10}(5678) \approx 3.7542$, so

\begin{align*}
    10^{\log_{10}(1234) + \log_{10}(5678)} \approx 10^{3.0913 + 3.7542} = 10^{6.8455}.
\end{align*}
...
\begin{align*}
   \frac{d}{dx} b^x = \Big( \frac{d}{dx} b^x \Big)\Big|_0 b^x = c(b) b^x, \text{ where $c(b) = \Big( \frac{d}{dx} b^x \Big)\Big|_0$}.
\end{align*}

That is, the derivative of $b^x$ with respect to $x$ is the evaluation of some function at $b$, times $b^x$. In order to obtain a more useful expression for $\frac{d}{dx} b^x$, we must evaluate the expression $c(b)$. A natural way to do so is to first consider the $b$ for which\footnote{Does such a $b$ even exist? Does $\Big( \frac{d}{dx} b^x \Big)\Big|_0 = \lim_{h \rightarrow 0} \frac{b^h - 1}{h}$ even converge for all $b$? The answer to both questions is yes, but showing so is quite technical.} we have $c(b) = 1$.

For the sake of having concrete notation, we will \textit{define} $e$ to be the real number such that $c(e) = 1$. Then

\begin{align*}
    \frac{d}{dx} e^x = c(e) e^x = 1 \cdot e^x = e^x.
\end{align*}
