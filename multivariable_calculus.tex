...
\begin{theorem}
    ...
    On page 58 of \textit{Calculus on Manifolds}, Spivak states that if $C \subseteq A \times B$, then

    \begin{align*}
        \int_C f &= \int_{A \times B} \chi_C f \\
        &= \int_A \Big( \int_B \chi_C(x, y) f(x, y) dy \Big) dx \\
        &= \int_B \Big( \int_A \chi_C(x, y) f(x, y) dy \Big) dx.
    \end{align*}

    On page 61, Spivak notes that to derive an expression for $\int_C f$, one will have to either derive an expression for either the ``slice'' $C \cap (\{x\} \times B)$ when $x \in A$ or the ``slice'' $C \cap (\{y\} \times A)$ when $y \in B$.
\end{theorem}

We can use Fubini's theorem to change the order of integration of integrals over nonrectangular regions by interpreting such integrals to be integrals over rectangular regions. In the below, we use the convention that when $S = \{\xx \in \R^n \mid P(\xx)\}$ we use $\chi(P(\xx))$ to denote $\chi_S$.

\begin{example}
    Consider the iterated integral

    \begin{align*}
        \int_{-1}^0 \left( \int_{-1}^y f(x, y) dx \right) dy.
    \end{align*}

    Since we are given a sequence of iterated integrals to start from, we already know an expression for slices taken perpendicular to the axis of the innermost variable, which is the $x$-axis in this case. The above tells us that for each $x$,
    
    \begin{align*}
        (\text{slice perpendicular to $x$-axis})(x) = C \cap (\{x\} \times A) = \{(x, y) \in \R^2 \mid -1 \leq x \leq y \leq 1 \}.
    \end{align*}

    Thus the above is

    \begin{align*}
        \int_{-1}^0 \left( \int_{-1}^0 \chi(-1 \leq x \leq y \leq 0) f(x, y) dx \right) dy &= \int_{-1}^0 \left( \int_{-1}^0 \chi(x \leq y) f(x, y) dx \right) dy \\
        &= \int_{-1}^0 \left( \int_{-1}^0 \chi(x \leq y) f(x, y) dy \right) dx \text{ by Fubini's theorem} \\
        &= \int_{-1}^0 \left( \int_{-1}^0 \chi(y \geq x) f(x, y) dy \right) dx \\
...
\begin{example}
    ...
    \begin{align*}
        \int_0^8 \left( \int_{\sqrt[3]{y}}^2 f(x, y) dx \right) dy.
    \end{align*}

    Since we are given a sequence of iterated integrals to start from, we already know an expression for slices taken perpendicular to the axis of the innermost variable, which is the $x$-axis in this case. The above tells us that for each $x$,
    
    \begin{align*}
        (\text{slice perpendicular to $x$-axis})(x) = C \cap (\{x\} \times A) = \{(x, y) \in \R^2 \mid \sqrt[3]{y} \leq x \leq 2, 0 \leq y \leq 8 \}.
    \end{align*}

    Thus the above is

    \begin{align*}
        \int_0^8 \left( \int_{\sqrt[3]{y}}^2 f(x, y) dx \right) dy &= \int_0^8 \left( \int_0^2 \chi(\sqrt[3]{y} \leq x) f(x, y) dx \right) dy \\
        &= \int_0^2 \left( \int_0^8 \chi(\sqrt[3]{y} \leq x) f(x, y) dy \right) dx \text{ by Fubini's theorem} \\
        &= \int_0^2 \left( \int_0^8 \chi(y \leq x^3) f(x, y) dy \right) dx \\
        &= \int_0^2 \int_0^{x^3} f(x, y) dy \spc dx.
    \end{align*}
\end{example}

Drawing pictures of the region of integration will help in determining the ``slices'' $C \cap (\{x\} \times B)$ when $x \in A$ and $C \cap (\{y\} \times A)$ when $y \in B$. It's a tricky matter, as for unusual regions some slices may be a union of two disjoint intervals.

I've always wondered if there were a more systematic way, though, to derive formulas for these slices. To better describe my wonderings, let me describe the situation more precisely. Given a volume $V \subseteq \R^n$, we want to compute the integral 
...
We have to determine the correct expressions for the bounds $u_{i_1}, \ell_{i_1}, ..., u_{i_k}, \ell_{i_k}$ to do so.
